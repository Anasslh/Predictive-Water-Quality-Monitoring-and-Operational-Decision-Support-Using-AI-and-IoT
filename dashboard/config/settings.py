...
WQD_DATA_SOURCE_MODE ``historical`` (default) | ``continuous``.
WQD_DATA_SOURCE_NOTE Optional deployment-specific source qualifier.
...
# Historical datasets do not participate in operational freshness/SLA alerts.
DATA_SOURCE_MODES = ("historical", "continuous")
...
class Settings:
    ...
    # Neutral qualifier shown under the data-source label. Empty string → the UI
    # falls back to the translated "Historical research data" default. Set to a
    # real site descriptor once the dashboard is connected to a live IoT source.
    data_source_note: str = field(default_factory=lambda: _env_str("WQD_DATA_SOURCE_NOTE", ""))
    data_source_mode: str = field(
        default_factory=lambda: _env_str("WQD_DATA_SOURCE_MODE", "historical")
    )
    ...
    # Continuous-mode freshness heuristic: a parameter is "stale" when the newest record is older
    ...
    def normalised(self) -> "Settings":
        """Return a copy with profile, language and source mode clamped."""
